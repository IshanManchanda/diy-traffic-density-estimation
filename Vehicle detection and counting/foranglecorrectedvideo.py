...
cap1= cv2.VideoCapture('/home/divyansh/Downloads/trafficfront.mp4')
...
video1 = cv2.VideoWriter('traffic_counter.avi1', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), fps1, (height21, width21), 1)
...
while True:
    ...
    if ret:  # if there is a frame continue with code
        ...
        for i in range(len(contours1)):  # cycles through all contours in current frame
            ...
            if hierarchy1[0, i, 3] == -1:  # using hierarchy to only count parent contours (contours not within others)
                ...
                if minarea < area1 < maxarea:  # area threshold for contour
                    ...
                    if ((cy1 > 0)):  # filters out contours that are above line (y starts at top)
                        ...
                        cv2.putText(image1, str(cx1) + "," + str(cy1), (cx1 + 10, cy1 + 10), cv2.FONT_HERSHEY_SIMPLEX,
                                    .3, (0, 0, 255), 1)
                ...
                for j in range(len(carids1)):  # loops through all current car ids
                    ...
                    if mindx1 == 0 and mindy1 == 0 and np.all(dx1[:, j] == 0) and np.all(dy1[:, j] == 0):
                        ...
                        if np.abs(mindx1) < maxrad1 and np.abs(mindy1) < maxrad1:
                            ...
                            minx_index21.append(minx_index1)  # appends all the indices that were added to previous carids
...
#         # The section below labels the centroids on screen
        ...
        for i in range(currentcars1):  # loops through all current car ids on screen
            ...
            if curcent1:  # if there is a current centroid
                ...
                if oldcent1:  # checks if old centroid exists
                    ...
                    cv2.rectangle(image1, (int(xstart1), int(ystart1)), (int(xwidth1), int(yheight1)), (0, 125, 0), 1)
        ...
        cv2.putText(image1, "Cars Entered: " + str(3+carscrossedup1), (0, 200), cv2.FONT_HERSHEY_SIMPLEX, .37, (0, 0, 255),
                    1)
